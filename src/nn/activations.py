...
def relu(h):
   ...
   def _backward():
      h.grad += _unbroadcast(relu_h.grad * (h.data > 0).astype(float), h.data.shape) # dL/dA = dL/dC * dC/dA = dL/dC * (A > 0)
...
def tanh(h) :
   ...
   def _backward():
      h.grad += _unbroadcast(tanh_h.grad * (1 - tanh_h.data ** 2), h.data.shape) # dL/dA = dL/dC * dC/dA = dL/dC * (1 - tanh(A)²)
...
def softmax(h):
   ...
   def _backward():
      s = softmax_h.data
      g = softmax_h.grad
      h.grad += _unbroadcast(s * (g - np.sum(g * s, axis=-1, keepdims=True)), h.data.shape) # dL/dA = dL/dS * dS/dA = S * (dL/dS - (dL/dS * S) 1_(n × n))
...
def swish(h): # Swish: https://arxiv.org/abs/1710.05941
   ...
   def _backward():
      ...
      h.grad += _unbroadcast(swish_h.grad * (swish_h.data + sigmoid_h * (1 - swish_h.data)), h.data.shape) # dL/dA = dL/dC * dC/dA = dL/dC * (f(A) + σ(A) * (1 - f(A)))
...
def gelu(h): # Gaussian Error Linear Unit: https://alaaalatif.github.io/2019-04-11-gelu/
   ...
   def _backward():
      ...
      h.grad += _unbroadcast(gelu_h.grad * gelu_grad, h.data.shape) 
